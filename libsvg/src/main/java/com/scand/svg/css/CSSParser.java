		...
		while (true) {
			...
			case ':': {
				...
				if (pseudoElement || name.equals("first-line") || name.equals("first-letter") || name.equals("before")
						|| name.equals("after") || name.equals("hover") || name.equals("active")|| name.equals("focus") 
						|| name.equals("link") || name.equals("visited") )
			...
			case '*':
				...
				break;
			default:
				if (('a' <= ch && ch <= 'z') || ('A' <= ch && ch <= 'Z') || ch == '_' || ch == '-' || ch >= 0x80 || ch == '\\' || ch == '|') {
					String ident;
					if (ch == '|')
						ident = null;
					else
			...
			ch = read();
			reset();
			if (ch < 0 || ch == '{' || ch == '>' || ch == '+' || ch == ',' || ch == ' ' || ch == '\t' || ch == '\r'	|| ch == '\n' || ch == '\f')
				return selector;
		}
	}
